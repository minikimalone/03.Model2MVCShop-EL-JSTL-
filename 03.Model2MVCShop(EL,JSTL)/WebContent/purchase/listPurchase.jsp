...
<table width="100%" border="0" cellspacing="0" cellpadding="0"	style="margin-top: 10px;">
	...
	<tr class="ct_list_pop">
		<td align="center">
			<a href="/getPurchase.do?tranNo=${p.tranNo}">${i.index+1}</a>
		</td>
		<td></td>
		<td align="left">
			<a href="/getUser.do?userId=${p.buyer.userId}">${p.buyer.userId}</a>
		</td>
		<td></td>
		<td align="left">${p.receiverName}</td>
		<td></td>
		<td align="left">${p.purchaseProd.prodName}</td>
		<td></td>
		...
		<c:if test="${! empty p.tranCode && (p.tranCode eq '2')}">
		<a href= "/updateTranCode.do?tranNo=${p.tranNo}&tranCode=3">물건도착</a>
		</c:if>
		<c:if test="${! empty p.tranCode && !(p.tranCode eq '2')}">
			 ---
		</c:if>
